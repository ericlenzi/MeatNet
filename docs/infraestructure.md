...
| **Production** | VPS *(pendiente de deploy)* | Supabase (PostgreSQL administrado) *(pendiente de deploy)* | `appsettings.Production.json` + variables de entorno |
...
| Versión en Supabase | La que ofrezca el proyecto (hoy puede ser 17) | Supabase |
| Schema | `meat` | `MeatContext` (`HasDefaultSchema`) |
| Extensión | `citext` | `MeatContext` (`HasPostgresExtension`) |
...
- **Schema `meat`, no `public`.** Supabase expone el schema `public` por su Data API (REST). El
  ...
  pueden quedar en un schema expuesto.
...
Condiciones que el diseño ya contempla:

- **Supabase — Data API deshabilitada.** No se usa, y expondría por REST cualquier tabla del schema
  `public`. Las tablas de MeatNet están en `meat`, que no se expone.
- **Supabase — conexión desde el VPS:** pooler Supavisor en *session mode* (puerto 5432) o conexión
  directa. **Evitar *transaction mode*** (puerto 6543): rompe los prepared statements de Npgsql.
- **Región:** `sa-east-1` (São Paulo) para Supabase y un VPS cercano. El Monitor de Faena refresca
  seguido y cada consulta suma la latencia de red.
- **Plan:** Pro para producción. El plan Free pausa el proyecto después de 7 días sin actividad y no
  tiene backups.
- **Migraciones en producción:** generar el script con `dotnet ef migrations script --idempotent` y
  aplicarlo en el deploy. Revisar antes el `context.Database.Migrate()` de `Program.cs`, que hoy las
  aplica al arrancar.
- **Zona horaria del VPS:** un servidor Linux corre en UTC y `DateTime.Now` devolvería 3 horas de más.
