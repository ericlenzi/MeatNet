...
import { getCatalogo, deleteCatalogoItem, ETIQUETAS } from '@/services/catalogosSimples.service'
...
// Una sola pantalla para los catalogos que son codigo + nombre: tipos de puesto y tipos de
// medicion. La ruta decide cual via la prop.
export default function CatalogoSimpleListPage({ catalogo }: { catalogo: CatalogoSimpleId }) {
  const navigate = useNavigate()
  const { toast } = useToast()
  const pagination = usePagination()
  const debouncedFilter = useDebounce(pagination.filter)
  const [statusFilter, setStatusFilter] = useState('active')
  const [data, setData] = useState<CatalogoSimple[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [sort, setSort] = useState<SortState | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<CatalogoSimple | null>(null)
  ...
  }, [catalogo, debouncedFilter, statusFilter, pagination.pageIndex, pagination.pageSize]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    // Al cambiar de catalogo se vuelve a la primera pagina: son listas distintas.
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteCatalogoItem(catalogo, deleteTarget.codigo)
      toast('success', etiqueta.singular + ' eliminado')
  ...
  const columns: Column<CatalogoSimple>[] = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/${catalogo}/${row.codigo}/edit`) }}
  ...
  return (
    <>
      <PageHeader title={etiqueta.plural}>
        <Button onClick={() => navigate(`/${catalogo}/create`)}>Nuevo</Button>
      </PageHeader>
      ...
      <ConfirmDialog
        ...
        message={`¿Esta seguro que desea eliminar "${deleteTarget?.nombre}"?`}
