...
// Categorias con las que opera la empresa. La categoria en si (codigo, nombre, especie, sexo)
// sale del catalogo global que mantiene el SUPERADMIN; aca se elige con cuales se trabaja y
// con que peso teorico y codigo de ERP.
export default function CategoriasHaciendaListPage() {
  ...
  const [data, setData] = useState<EmpresaTipoEspecie[]>([])
  const [especies, setEspecies] = useState<Especie[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [sort, setSort] = useState<SortState | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<EmpresaTipoEspecie | null>(null)
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true)
    try {
      const response = await getEmpresasTiposEspecies({
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteEmpresaTipoEspecie(deleteTarget.id)
      toast('success', 'Categoria quitada')
  ...
  const columns: Column<EmpresaTipoEspecie>[] = [
    { key: 'tipoEspecieId', header: 'Codigo', width: '110px', sortable: true },
    { key: 'nombre', header: 'Nombre', sortable: true },
    { key: 'especieNombre', header: 'Especie', width: '130px', sortable: true },
    { key: 'tipoSexoNombre', header: 'Sexo', width: '100px', sortable: true },
    { key: 'pesoTeorico', header: 'Peso Teorico', width: '110px' },
    {
      key: 'pesoTeoricoReferencia',
      header: 'Peso Ref.',
      width: '110px',
      render: (value, row) => (
        <span className={row.pesoTeorico !== value ? 'text-text-light' : ''}>{String(value)}</span>
      ),
    },
    { key: 'erP_Codigo', header: 'Cod. ERP', width: '110px' },
    {
      ...
      sortable: true,
      render: (value, row) =>
        !row.tipoEspecieActivo ? (
          <Badge variant="danger">Baja en catalogo</Badge>
        ) : (
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/categorias-hacienda/${row.id}/edit`) }}
  ...
  const mensajeBaja =
    'Confirma que la empresa deja de operar con "' +
    (deleteTarget?.nombre ?? '') +
    '"? Si la categoria ya se uso, conviene desactivarla en lugar de quitarla.'

  return (
    <>
      <PageHeader title="Categorias de Hacienda">
        <Button onClick={() => navigate('/categorias-hacienda/create')}>Agregar Categoria</Button>
      </PageHeader>
