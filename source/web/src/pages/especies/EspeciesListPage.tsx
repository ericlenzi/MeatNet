...
import { getEspecies, deleteEspecie } from '@/services/especies.service'
...
export default function EspeciesListPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Especie | null>(null)
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteEspecie(deleteTarget.codigo)
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/especies/${row.codigo}/edit`) }}
  ...
  return (
    <>
      <PageHeader title="Especies">
        <Button onClick={() => navigate('/especies/create')}>Nueva Especie</Button>
      </PageHeader>
      ...
      <ConfirmDialog
        ...
        message={`¿Esta seguro que desea eliminar la especie "${deleteTarget?.nombre}"?`}
