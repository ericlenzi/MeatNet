...
import { getMateriales, deleteMaterial, getTiposMateriales } from '@/services/materiales.service'
...
export default function MaterialesListPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await getMateriales({
        ...
        TipoMaterialId: tipoFilter || undefined,
  ...
  }, [debouncedFilter, statusFilter, tipoFilter, pagination.pageIndex, pagination.pageSize]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const columns: Column<Material>[] = [
    { key: 'codigoMaterial', header: 'Codigo', width: '130px' },
    { key: 'nombre', header: 'Nombre' },
    { key: 'tipoMaterialNombre', header: 'Tipo', width: '130px' },
    { key: 'unidadMedidaNombre', header: 'U.M.', width: '90px' },
    { key: 'erP_Codigo', header: 'Cod. ERP', width: '120px' },
    {
      key: 'activo',
      header: 'Estado',
      width: '100px',
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/materiales/${row.id}/edit`) }}
  ...
  return (
    <>
      <PageHeader title="Materiales">
        <Button onClick={() => navigate('/materiales/create')}>Nuevo Material</Button>
      </PageHeader>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <SearchInput
          ...
          placeholder="Buscar por nombre, codigo, cod. ERP..."
        />
        <div className="flex items-center gap-2">
          <Select
            ...
            options={[
              ...
              ...tipos.map((t) => ({ value: t.codigo, label: t.nombre })),
            ]}
          />
      ...
      <ConfirmDialog
        ...
        message={`¿Esta seguro que desea eliminar el material "${deleteTarget?.nombre}"?`}
