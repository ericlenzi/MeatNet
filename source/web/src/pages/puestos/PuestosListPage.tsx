...
import { getPuestos, deletePuesto } from '@/services/puestos.service'
...
import type { Puesto, Especie, Establecimiento } from '@/types'
...
export default function PuestosListPage() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deletePuesto(deleteTarget.id)
  ...
  const columns: Column<Puesto>[] = [
    { key: 'codigoPuesto', header: 'Codigo', width: '100px' },
    { key: 'nombre', header: 'Nombre' },
    { key: 'establecimientoNombre', header: 'Establecimiento', width: '200px' },
    { key: 'especieNombre', header: 'Especie', width: '130px' },
    { key: 'tipoPuestoNombre', header: 'Tipo', width: '110px' },
    { key: 'tipoMedicionNombre', header: 'Medicion', width: '120px' },
    {
      key: 'activo',
      header: 'Estado',
      width: '100px',
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/puestos/${row.id}/edit`) }}
  ...
  return (
    <>
      <PageHeader title="Puestos">
        <Button onClick={() => navigate('/puestos/create')}>Nuevo Puesto</Button>
      </PageHeader>
      ...
      <ConfirmDialog
        ...
        message={`¿Esta seguro que desea eliminar el puesto "${deleteTarget?.nombre}"?`}
