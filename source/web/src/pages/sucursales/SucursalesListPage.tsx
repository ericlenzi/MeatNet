...
import { getSucursales, deleteSucursal } from '@/services/sucursales.service'
...
export default function SucursalesListPage() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteSucursal(deleteTarget.id)
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/sucursales/${row.id}/edit`) }}
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        message={`¿Esta seguro que desea eliminar la sucursal "${deleteTarget?.nombre}"?`}
