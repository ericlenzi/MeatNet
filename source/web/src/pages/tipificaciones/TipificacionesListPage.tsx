...
import type { Tipificacion } from '@/types'
...
export default function TipificacionesListPage() {
  const navigate = useNavigate()
  const { toast } = useToast()
  const pagination = usePagination()
  const debouncedFilter = useDebounce(pagination.filter)
  const [statusFilter, setStatusFilter] = useState('active')
  const [data, setData] = useState<Tipificacion[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [sort, setSort] = useState<SortState | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<Tipificacion | null>(null)
  const [isDeleting, setIsDeleting] = useState(false)

  const fetchData = useCallback(async () => {
    setIsLoading(true)
    try {
      const response = await getTipificaciones({
  ...
  const columns: Column<Tipificacion>[] = [
    ...
    { key: 'unidadFaenaNombre', header: 'Unidad Faena', width: '130px', render: (v) => (v ? String(v) : '-') },
    {
      ...
      header: 'Peso',
      width: '110px',
      render: (_, row) => `${row.pesoDesde} - ${row.pesoHasta}`,
    },
    { key: 'puntos', header: 'Puntos', width: '80px' },
    {
      key: 'activo',
      header: 'Estado',
      width: '100px',
  ...
  return (
    <>
      <PageHeader title="Tipificaciones">
        <Button onClick={() => navigate('/tipificaciones/create')}>Nueva Tipificacion</Button>
      </PageHeader>
      ...
      <ConfirmDialog
        ...
        message={`¿Esta seguro que desea eliminar la tipificacion "${deleteTarget?.codigo}"?`}
