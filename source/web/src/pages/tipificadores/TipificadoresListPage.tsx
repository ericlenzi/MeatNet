...
import { getTipificadores, deleteTipificador } from '@/services/tipificadores.service'
...
import type { Tipificador, Especie, Establecimiento } from '@/types'
...
export default function TipificadoresListPage() {
  ...
  const [data, setData] = useState<Tipificador[]>([])
  const [establecimientos, setEstablecimientos] = useState<Establecimiento[]>([])
  const [especies, setEspecies] = useState<Especie[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [sort, setSort] = useState<SortState | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<Tipificador | null>(null)
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true)
    try {
      const response = await getTipificadores({
        Filter: debouncedFilter || undefined,
        Estado: statusFilter === 'active' ? true : statusFilter === 'inactive' ? false : undefined,
        EstablecimientoId: establecimientoFilter || undefined,
      ...
      toast('error', 'Error al cargar tipificadores')
    } finally {
      setIsLoading(false)
    }
  }, [debouncedFilter, statusFilter, establecimientoFilter, especieFilter, pagination.pageIndex, pagination.pageSize]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteTipificador(deleteTarget.id)
  ...
  const columns: Column<Tipificador>[] = [
    { key: 'matricula', header: 'Matricula', width: '120px' },
    { key: 'nombre', header: 'Nombre' },
    { key: 'establecimientoNombre', header: 'Establecimiento', width: '200px' },
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/tipificadores/${row.id}/edit`) }}
  ...
  return (
    <>
      <PageHeader title="Tipificadores">
        <Button onClick={() => navigate('/tipificadores/create')}>Nuevo Tipificador</Button>
      </PageHeader>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Select
            ...
            onChange={(e) => handleFilterChange(setEstablecimientoFilter)(e.target.value)}
            options={[
      ...
      <ConfirmDialog
        ...
        title="Eliminar tipificador"
        message={`¿Esta seguro que desea eliminar a "${deleteTarget?.nombre}"?`}
